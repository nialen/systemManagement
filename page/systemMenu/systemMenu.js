...
angular
	...
	.run(['$rootScope', function($rootScope) {
		$rootScope.isMock = true; // 是否MOCK数据
		$rootScope.checkedSysModular = {}; // 选中的业务模块ID
		$rootScope.sysModularId = ''; // 选中的业务模块IDID
	}])
	...
	.controller('configurationMenu', ['$scope', '$log', 'httpMethod', function($scope, $log, httpMethod) {
	...
	.controller('configurationEditFormCtrl', ['$scope', '$log', 'httpMethod', function($scope, $log, httpMethod){
		
		$scope.configurationEditForm = {
			sysMenuId: '', // 菜单ID
			sysMenuName: '', // 菜单名称
			orderSeq: '', // 展示顺序
			// sysModularId: '', // 业务模块ID
		...
		$scope.configurationEditFormSubmit = function() {
			httpMethod.editMenu($scope.configurationEditForm).then(function(rsp) {
				$log.log('调用修改菜单接口成功.');
				if (rsp.data) {
					swal("操作成功!", "修改菜单成功！", "success");
				} else {
	...
	.controller('sysResultListCtrl', ['$scope', '$rootScope', '$log', function($scope, $rootScope, $log) {
		...
		$scope.sysModularIdFormSubmit = function() {
			$rootScope.sysModularId = $rootScope.checkedSysModular.sysModularId;
		}
	}])
	// 弹出框控制器
